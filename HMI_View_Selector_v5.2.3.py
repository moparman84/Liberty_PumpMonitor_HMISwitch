...
class ModernApp:
    def __init__(self, root):
        ...
        self.auto_restart_active = False  # Auto-restart feature toggle
        self.auto_restart_monitoring = False  # Auto-restart monitoring thread active
        self.auto_restart_analog_channel = 1  # Selected analog channel (1-32)
        self.auto_restart_monitor_ip = "10.55.0.10"  # IP to monitor for analog values
        self.auto_restart_address = 405500  # Base Modbus address for analog channels
        self.auto_restart_enable_threshold = 4000  # Enable restart when value > 4000
        self.auto_restart_disable_threshold = 3000  # Disable restart when value < 3000
        self.auto_restart_thread = None  # Thread for monitoring analog values
        self.auto_restart_last_state = False  # Track last restart state to avoid spam
        
        # RPM-based auto-restart tracking per unit
        self.unit_restart_states = {}  # Track restart state for each unit IP
        # Possible states: 'normal', 'restarting', 'waiting_env_state', 'waiting_start', 'ignored', 'start_failed'
    ...
    def stop_monitoring(self):
        ...
        # Also stop auto-restart monitoring when stopping regular monitoring
        if self.auto_restart_active:
            self.stop_auto_restart_monitoring()
    ...
    def create_operations_page(self):
        ...
        # Auto-Restart button
        auto_restart_button = HoverButton(
            button_frame,
            text="Auto-Restart",
            width=15,
            font=("Segoe UI", 12, "bold"),
            ...
            relief="flat",
            hover_color="#7b79e6",
            command=self.create_auto_restart_page
        )
        auto_restart_button.pack(side='left', padx=10, ipady=5)
    ...
    def toggle_auto_restart(self):
        """Toggle the Auto-Restart functionality on/off"""
        self.auto_restart_active = not self.auto_restart_active
        
        if self.auto_restart_active:
            self.auto_restart_button.config(text="Auto-Restart: ON", bg="#107c10", hover_color="#25902a")
            self.auto_restart_status_label.config(text="Status: Monitoring...", fg='#00ff00')
            self.start_auto_restart_monitoring()
        else:
            self.auto_restart_button.config(text="Auto-Restart: OFF", bg="#6b69d6", hover_color="#7b79e6")
            self.auto_restart_status_label.config(text="Status: Disabled", fg='#aaaaaa')
            self.stop_auto_restart_monitoring()
    
    def on_analog_channel_change(self, event=None):
        """Handle analog channel selection change"""
        selected = self.analog_dropdown.get()
        # Extract channel number from "Analog X" format
        channel_num = int(selected.split()[1])
        self.auto_restart_analog_channel = channel_num
        print(f"Auto-Restart analog channel changed to: {channel_num}")
    
    def start_auto_restart_monitoring(self):
        """Start monitoring the analog channel for auto-restart triggers"""
        if self.auto_restart_monitoring:
            return
            
        self.auto_restart_monitoring = True
        self.auto_restart_thread = threading.Thread(target=self.auto_restart_monitor_loop, daemon=True)
        self.auto_restart_thread.start()
        print(f"Started Auto-Restart monitoring on Analog {self.auto_restart_analog_channel}")
    
    def stop_auto_restart_monitoring(self):
        """Stop the auto-restart monitoring"""
        self.auto_restart_monitoring = False
        if self.auto_restart_thread and self.auto_restart_thread.is_alive():
            self.auto_restart_thread.join(timeout=1)
        print("Stopped Auto-Restart monitoring")
    
    def auto_restart_monitor_loop(self):
        """Main monitoring loop for auto-restart functionality"""
        while self.auto_restart_monitoring:
            try:
                # Calculate the Modbus address for the selected analog channel
                # Address = base_address + (channel - 1) * 2 (each analog channel uses 2 registers)
                address = self.auto_restart_address + (self.auto_restart_analog_channel - 1) * 2
                
                # Connect to the monitoring IP
                client = ModbusTcpClient(self.auto_restart_monitor_ip)
                if client.connect():
                    # Read the analog value (2 registers for 32-bit float/long)
                    result = client.read_holding_registers(address=address, count=2)
                    if not result.isError():
                        # Convert registers to value (assuming 32-bit integer)
                        ...
                        print(f"Auto-Restart: Analog {self.auto_restart_analog_channel} = {raw_value}")
                        
                        # Update analog value display if on auto-restart page
                        if hasattr(self, 'analog_value_display'):
                            self.safe_widget_update(self.analog_value_display, text=str(raw_value))
                        
                        # Check thresholds and update restart state
                        if raw_value > self.auto_restart_enable_threshold and not self.auto_restart_last_state:
                            # Enable auto-restart
                            self.auto_restart_last_state = True
                            self.safe_widget_update(self.auto_restart_status_label, 
                                                  text=f"Status: ACTIVE (Value: {raw_value})", fg='#ff0000')
                            print(f"Auto-Restart ENABLED: Value {raw_value} > {self.auto_restart_enable_threshold}")
                            
                        elif raw_value < self.auto_restart_disable_threshold and self.auto_restart_last_state:
                            # Disable auto-restart
                            self.auto_restart_last_state = False
                            self.safe_widget_update(self.auto_restart_status_label, 
                                                  text=f"Status: Monitoring (Value: {raw_value})", fg='#00ff00')
                            print(f"Auto-Restart DISABLED: Value {raw_value} < {self.auto_restart_disable_threshold}")
                        
                        # If restart is active and we need to restart units
                        if self.auto_restart_last_state:
                            self.execute_auto_restart()
                    
                    client.close()
                else:
                    print(f"Failed to connect to monitoring IP: {self.auto_restart_monitor_ip}")
                    self.safe_widget_update(self.auto_restart_status_label, 
                                          text="Status: Connection Error", fg='#ff0000')
                    
            except Exception as e:
                print(f"Error in auto-restart monitoring: {e}")
                self.safe_widget_update(self.auto_restart_status_label, 
                                      text="Status: Error", fg='#ff0000')
            
            # Wait before next check
            time.sleep(5)  # Check every 5 seconds
    
    def execute_auto_restart(self):
        """Execute restart for all 230xx units when auto-restart is active"""
        try:
            # Get only 230xx units (filter out LFPC)
            units_230xx = [unit for unit in self.units_info if unit['unit_name'].startswith('230')]
            
            if not units_230xx:
                print("No 230xx units found for auto-restart")
                return
            
            print(f"Executing auto-restart for {len(units_230xx)} units: {[u['unit_name'] for u in units_230xx]}")
            
            # Restart each 230xx unit via SSH (similar to the Controls page functionality)
            for unit in units_230xx:
                try:
                    self.restart_unit_ssh(unit)
                except Exception as e:
                    print(f"Failed to restart unit {unit['unit_name']}: {e}")
                    
        except Exception as e:
            print(f"Error in execute_auto_restart: {e}")
    
    def restart_unit_ssh(self, unit):
        """Restart a single unit via SSH"""
        unit_name = unit['unit_name']
        ip_address = unit['ip_address']
        ...
        try:
            ...
            # Connect with credentials (using same as Controls page)
            ssh.connect(ip_address, username='admin', password='LBRT123!', timeout=10)
            
            # Send reboot command
            stdin, stdout, stderr = ssh.exec_command('sudo reboot')
            
            print(f"Auto-restart command sent to {unit_name} ({ip_address})")
            
            ssh.close()
            
        except Exception as e:
            print(f"SSH restart failed for {unit_name} ({ip_address}): {e}")
            raise

    def create_auto_restart_page(self):
        """Create the Auto-Restart page with unit containers and monitoring controls"""
        # Track current monitoring state before stopping
        self.was_monitoring_before_navigation = self.monitoring_active
        ...
        # Find only 230xx folders for auto-restart (LFPC units are excluded)
        self.units_info = self.find_230xx_folders()
        ...
        # Create a frame for the units display with modern styling
        self.auto_restart_grid_frame = tk.Frame(self.current_frame, bg='#1e1e1e')
        self.auto_restart_grid_frame.pack(expand=True, fill='both', padx=10, pady=10)
        
        # Create monitor displays for each unit with auto-restart data
        self.create_auto_restart_monitors()
        ...
        if self.auto_restart_active:
            button_text = "Auto-Restart: ON"
            button_bg = "#107c10"  # Green when active
            hover_color = "#25902a"  # Lighter green on hover
        else:
            button_text = "Auto-Restart: OFF"
            button_bg = "#6b69d6"  # Purple when inactive
            hover_color = "#7b79e6"  # Lighter purple on hover
            
        self.auto_restart_button = HoverButton(
            button_frame,
            text=button_text,
            width=18,
            font=("Segoe UI", 12, "bold"),
        ...
        self.auto_restart_button.pack(side='left', padx=10, ipady=5)
        
        # Analog channel selection
        analog_label = tk.Label(
            button_frame,
            text="Analog Channel:",
            font=("Segoe UI", 12, "bold"),
            bg='#1e1e1e',
        ...
        analog_label.pack(side='left', padx=(10, 5))
        
        # Analog channel dropdown
        analog_values = [f"Analog {i}" for i in range(1, 33)]
        self.analog_dropdown = ttk.Combobox(
            button_frame,
        ...
        self.analog_dropdown.set("Analog 1")  # Default selection
        self.analog_dropdown.bind("<<ComboboxSelected>>", self.on_analog_channel_change)
        self.analog_dropdown.pack(side='left', padx=5)
        
        # Analog value display
        analog_value_label = tk.Label(
            button_frame,
            text="Value:",
            font=("Segoe UI", 12, "bold"),
            bg='#1e1e1e',
        ...
        analog_value_label.pack(side='left', padx=(20, 5))
        
        # Digital display for analog value
        self.analog_value_display = tk.Label(
            button_frame,
            text="----",
            font=("Segoe UI", 14, "bold"),
            width=8,
        ...
        # Start monitoring button
        self.auto_restart_start_button = HoverButton(
            button_frame,
            text="Start Monitor",
            width=15,
            font=("Segoe UI", 12, "bold"),
            ...
            relief="flat",
            hover_color="#25902a",
            command=self.start_auto_restart_monitoring_page
        )
        self.auto_restart_start_button.pack(side='left', padx=10, ipady=5)
        
        # Stop monitoring button
        self.auto_restart_stop_button = HoverButton(
            button_frame,
            text="Stop Monitor",
            width=15,
            font=("Segoe UI", 12, "bold"),
            ...
            relief="flat",
            hover_color="#e85b24",
            command=self.stop_auto_restart_monitoring_page
        )
        self.auto_restart_stop_button.pack(side='left', padx=10, ipady=5)
        
        # Status label for Auto-Restart
        ...
        self.auto_restart_status_label.pack(side='left', padx=(20, 0))
        ...
        back_button.pack(side='right', padx=10, ipady=5)

    def create_auto_restart_monitors(self):
        """Create monitoring displays for each unit with auto-restart data"""
        # Load pump assignments to map pump numbers to unit numbers
        pump_assignments = self.load_assignments()
        ...
        # Calculate rows and columns (6 rows per column, same as turbo monitor)
        rows_per_column = 6
        num_units = len(all_units)
        num_columns = (num_units + rows_per_column - 1) // rows_per_column
        ...
        for i, unit in enumerate(all_units):
            ...
            # Create a frame for this unit (same size as turbo monitor)
            unit_frame = tk.Frame(self.auto_restart_grid_frame, bg='#2d2d2d', padx=7, pady=7, bd=1, relief='solid', width=200, height=180)
            unit_frame.grid(row=row, column=col, padx=5, pady=5, sticky='nsew')
            unit_frame.grid_propagate(False)  # Force the frame to maintain its size
            ...
            # Env State and RMP display (same row)
            env_rmp_frame = tk.Frame(unit_frame, bg='#2d2d2d')
            env_rmp_frame.pack(fill='x', pady=2)
            ...
            env_value.pack(side='left', padx=(5, 10))
            ...
            rmp_label.pack(side='left')
            
            # Digital display for RMP
            rmp_value = tk.Label(
                env_rmp_frame,
                text="---",
                font=("Segoe UI", 9, "bold"),
            ...
            rmp_value.pack(side='left', padx=5)
            
            # V1, V2 indicators and Status display (same row)
            indicators_frame = tk.Frame(unit_frame, bg='#2d2d2d')
            ...
            v1_indicator.pack(side='left', padx=(5, 10))
            ...
            v2_indicator.pack(side='left', padx=(5, 10))
            
            # Status display (same row as V1/V2)
            status_label = tk.Label(
                indicators_frame,
                text="Status:",
                font=("Segoe UI", 8),
            ...
            # Initialize restart state for this unit
            self.unit_restart_states[unit['ip_address']] = {
                'state': 'normal',
                'timestamp': 0,
                'rpm_below_threshold_time': 0,
                'restart_attempts': 0
            }
            
            # Store widget references for updates
            unit['auto_restart_widgets'] = {
    ...
    def start_auto_restart_monitoring_page(self):
        """Start monitoring for the auto-restart page"""
        if self.monitoring_active:
            return
            
        self.monitoring_active = True
        print("Starting Auto-Restart page monitoring...")
        
        # Start monitoring thread for each unit
        for unit in self.units_info:
            thread = threading.Thread(target=self.monitor_auto_restart_unit, args=(unit,), daemon=True)
            thread.start()
            self.monitor_threads.append(thread)
        
        # Start analog channel monitoring
        if self.auto_restart_active:
            self.start_auto_restart_monitoring()
        
        # Update button states
        try:
            if hasattr(self, 'auto_restart_start_button'):
                self.auto_restart_start_button.config(state='disabled')
            if hasattr(self, 'auto_restart_stop_button'):
                self.auto_restart_stop_button.config(state='normal')
        except (tk.TclError, RuntimeError, AttributeError):
            pass

    def stop_auto_restart_monitoring_page(self):
        """Stop monitoring for the auto-restart page"""
        self.stop_monitoring()
        self.stop_auto_restart_monitoring()
        ...
        try:
            if hasattr(self, 'auto_restart_start_button'):
                self.auto_restart_start_button.config(state='normal')
            if hasattr(self, 'auto_restart_stop_button'):
                self.auto_restart_stop_button.config(state='disabled')
        except (tk.TclError, RuntimeError, AttributeError):
            pass

    def monitor_auto_restart_unit(self, unit):
        """Monitor Modbus registers for a specific unit on auto-restart page with RPM-based restart logic"""
        ip = unit['ip_address']
        unit_name = unit.get('unit_name', 'Unknown')
        print(f"Starting auto-restart monitoring thread for unit {unit_name} at {ip}")
        
        try:
            widgets = unit['auto_restart_widgets']
        except KeyError:
            print(f"No widgets found for unit {unit_name}")
            return
        ...
        while self.monitoring_active:
            try:
                ...
                if client.connect():
                    current_time = time.time()
                    restart_state = self.unit_restart_states.get(ip, {'state': 'normal', 'timestamp': 0, 'rpm_below_threshold_time': 0, 'restart_attempts': 0})
                    
                    # Read all required registers
                    # Env State (302001.00-07) - bits 0-7 of register 302001 -> 2001
                    env_result = client.read_input_registers(address=2001, count=1)
                    env_state = None
                    rmp_value = None
                    
                    if not env_result.isError():
                        env_state = env_result.registers[0] & 0xFF  # Get lower 8 bits
                        rmp_value = (env_result.registers[0] >> 8) & 0xFF  # Get upper 8 bits (RPM)
                        self.safe_widget_update(widgets.get('env_value'), text=str(env_state))
                        self.safe_widget_update(widgets.get('rmp_value'), text=str(rmp_value))
                    ...
                    if not v1_result.isError():
                        v1_state = bool(v1_result.registers[0] & (1 << 5))
                        v2_state = bool(v1_result.registers[0] & (1 << 6))
                        bit_10_state = bool(v1_result.registers[0] & (1 << 10))  # Bit 10 for restart condition
                        
                        v1_color = '#00ff00' if v1_state else 'gray'
                        v2_color = '#00ff00' if v2_state else 'gray'
                        self.safe_widget_update(widgets.get('v1_indicator'), fg=v1_color)
                        self.safe_widget_update(widgets.get('v2_indicator'), fg=v2_color)
                    
                    # Auto-restart logic (only if auto-restart is enabled)
                    if self.auto_restart_active and rmp_value is not None and env_state is not None:
                        self.process_auto_restart_logic(ip, unit_name, rmp_value, env_state, bit_10_state, current_time, client, widgets)
                    
                    client.close()
                    
            except Exception as e:
                print(f"Error monitoring auto-restart unit {unit['unit_name']} at {ip}: {e}")
                # Update all widgets to show error state
                for widget_name, widget in widgets.items():
                    if 'indicator' in widget_name:
                        self.safe_widget_update(widget, fg='red')
                    elif widget_name == 'status_display':
                        self.safe_widget_update(widget, text="ERROR", fg='red')
                    else:
                        self.safe_widget_update(widget, text="ERR")
    ...
    def process_auto_restart_logic(self, ip, unit_name, rpm, env_state, bit_10_state, current_time, client, widgets):
        """Process the complex auto-restart logic for a unit"""
        restart_state = self.unit_restart_states[ip]
        state = restart_state['state']
        timestamp = restart_state['timestamp']
        
        try:
            if state == 'normal':
                # Check if RPM drops below 1000
                if rpm < 1000:
                    if restart_state['rpm_below_threshold_time'] == 0:
                        restart_state['rpm_below_threshold_time'] = current_time
                        print(f"{unit_name}: RPM below 1000 ({rpm}), starting timer...")
                    elif current_time - restart_state['rpm_below_threshold_time'] >= 5:  # 5 seconds below threshold
                        # Start restart sequence
                        print(f"{unit_name}: Starting restart sequence - setting address 400285 to 0")
                        self.write_modbus_registers(client, 400285, [0, 0])  # Set to 0 (2 registers)
                        restart_state['state'] = 'restarting'
                        restart_state['timestamp'] = current_time
                        restart_state['restart_attempts'] += 1
                        self.safe_widget_update(widgets.get('status_display'), text="Restarting", fg='#ffaa00')
                else:
                    # RPM is above threshold, reset timer
                    restart_state['rpm_below_threshold_time'] = 0
            
            elif state == 'restarting':
                # Wait 60 seconds after setting 400285 to 0
                if current_time - timestamp >= 60:
                    print(f"{unit_name}: 60 seconds elapsed, checking Env State")
                    if env_state == 5:
                        # Env State is 5, check bit 10
                        if bit_10_state:
                            print(f"{unit_name}: Env State = 5 and bit 10 is true, setting 400285 to 1")
                            self.write_modbus_registers(client, 400285, [1, 0])  # Set to 1
                            restart_state['state'] = 'waiting_start'
                            restart_state['timestamp'] = current_time
                            self.safe_widget_update(widgets.get('status_display'), text="Starting", fg='#00aaff')
                        else:
                            print(f"{unit_name}: Env State = 5 but bit 10 is false, continuing to wait")
                    else:
                        # Env State is not 5, reset sequence
                        print(f"{unit_name}: Env State != 5 ({env_state}), performing reset sequence")
                        self.write_modbus_registers(client, 401209, [0, 0])  # Set 401209 to 0
                        restart_state['state'] = 'waiting_env_state'
                        restart_state['timestamp'] = current_time
                        self.safe_widget_update(widgets.get('status_display'), text="Resetting", fg='#ff6600')
            
            elif state == 'waiting_env_state':
                # Wait 10 seconds after setting 401209 to 0
                if current_time - timestamp >= 10:
                    print(f"{unit_name}: Setting 401209 to 1 after 10 second wait")
                    self.write_modbus_registers(client, 401209, [1, 0])  # Set 401209 to 1
                    restart_state['state'] = 'waiting_env_state_5'
                    restart_state['timestamp'] = current_time
                    self.safe_widget_update(widgets.get('status_display'), text="Wait Env=5", fg='#ff6600')
            
            elif state == 'waiting_env_state_5':
                # Wait up to 5 minutes for Env State = 5
                if env_state == 5:
                    if bit_10_state:
                        print(f"{unit_name}: Env State = 5 and bit 10 is true, setting 400285 to 1")
                        self.write_modbus_registers(client, 400285, [1, 0])  # Set to 1
                        ...
                        print(f"{unit_name}: Env State = 5 but bit 10 is false, continuing to wait")
                elif current_time - timestamp >= 300:  # 5 minutes timeout
                    print(f"{unit_name}: Timeout waiting for Env State = 5, restart failed")
                    restart_state['state'] = 'start_failed'
                    restart_state['timestamp'] = current_time
                    self.safe_widget_update(widgets.get('status_display'), text="Start Failed", fg='#ff0000')
            
            elif state == 'waiting_start':
                # Wait 10 minutes for pump to start
                if current_time - timestamp >= 600:  # 10 minutes
                    if rpm == 0:
                        print(f"{unit_name}: RPM still 0 after 10 minutes, setting 400285 to 0")
                        self.write_modbus_registers(client, 400285, [0, 0])  # Set to 0
                        restart_state['state'] = 'start_failed'
                        restart_state['timestamp'] = current_time
                        self.safe_widget_update(widgets.get('status_display'), text="Start Failed", fg='#ff0000')
                    else:
                        print(f"{unit_name}: Pump started successfully (RPM: {rpm})")
                        restart_state['state'] = 'ignored'
                        restart_state['timestamp'] = current_time
                        restart_state['rpm_below_threshold_time'] = 0
                        self.safe_widget_update(widgets.get('status_display'), text="Started", fg='#00ff00')
            
            elif state == 'ignored':
                # Ignore for 10 minutes after successful start
                if current_time - timestamp >= 600:  # 10 minutes
                    print(f"{unit_name}: Ignore period ended, returning to normal monitoring")
                    restart_state['state'] = 'normal'
                    restart_state['timestamp'] = 0
                    restart_state['rpm_below_threshold_time'] = 0
                    self.safe_widget_update(widgets.get('status_display'), text="Normal", fg='#00ff00')
            
            elif state == 'start_failed':
                # Stay in failed state until manual reset or RPM recovery
                if rpm >= 1000:
                    print(f"{unit_name}: RPM recovered, returning to normal")
                    restart_state['state'] = 'normal'
                    restart_state['timestamp'] = 0
                    restart_state['rpm_below_threshold_time'] = 0
                    self.safe_widget_update(widgets.get('status_display'), text="Normal", fg='#00ff00')
            ...
            self.unit_restart_states[ip] = restart_state
            
        except Exception as e:
            print(f"Error in auto-restart logic for {unit_name}: {e}")
            restart_state['state'] = 'normal'
            restart_state['timestamp'] = 0
            self.safe_widget_update(widgets.get('status_display'), text="Error", fg='#ff0000')

    def write_modbus_registers(self, client, address, values):
        ...
        try:
            ...
            if result.isError():
                ...
                return False
            print(f"Successfully wrote {values} to address {address}")
            return True
        except Exception as e:
    ...
    def create_controls_page(self):
        ...
        header_label = tk.Label(
            header_frame,
            text="Controls - Log Download & System Management",
            font=("Segoe UI", 26, "bold"),
            bg='#1e1e1e',
            fg='white'
        )
    ...
    def download_unit_logs(self, unit):
        """Download logs from /c/jobs/ folder via SSH and organize them"""
        unit_name = unit['unit_name']
        ip_address = unit['ip_address']
        ...
        def download_task():
            try:
                ...
                # Check if remote directory exists and list files
                try:
                ...
                # Update progress
                self.root.after(0, lambda: progress_label.config(text=f"Downloading {len(file_list)} files..."))
                ...
                # Update progress
                self.root.after(0, lambda: progress_label.config(text="Deleting remote files..."))
                ...
                if remaining_files:
                    message = f"Downloaded {len(downloaded_files)} files to:\n{download_dir}\n\nWarning: {len(remaining_files)} files remain in remote directory."
                else:
                    message = f"Successfully downloaded {len(downloaded_files)} files to:\n{download_dir}\n\nRemote directory cleared."
    ...
    def reboot_unit(self, unit):
        """Reboot the unit via SSH after confirming /c/jobs/ is empty"""
        unit_name = unit['unit_name']
        ip_address = unit['ip_address']
        ...
        # Show progress dialog
        progress_window = tk.Toplevel(self.root)
        progress_window.title(f"Rebooting - {unit_name}")
        progress_window.geometry("400x150")
        progress_window.configure(bg='#1e1e1e')
        progress_window.transient(self.root)
        progress_window.grab_set()
        ...
        def reboot_task():
            try:
                ...
                # Create SFTP client to check directory
                sftp = ssh.open_sftp()
                
                # Update progress
                self.root.after(0, lambda: progress_label.config(text="Verifying /c/jobs/ is empty..."))
                
                # Check if remote directory is empty
                try:
                    file_list = sftp.listdir(remote_path)
                    if file_list:
                        self.root.after(0, lambda: self.show_message("Warning", f"/c/jobs/ directory is not empty on {unit_name}.\nFound {len(file_list)} files. Please download logs first."))
                        return
                except FileNotFoundError:
                ...
                # Show completion message
                self.root.after(0, lambda: self.show_message("Success", f"Reboot command sent to {unit_name}.\nThe unit should restart shortly."))
