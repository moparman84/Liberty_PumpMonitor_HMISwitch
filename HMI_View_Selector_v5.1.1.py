...
class ModernApp:
    def __init__(self, root):
        ...
        self.maintenance_password = "LBRT123!"  # Default password
        self.turbo_temp_threshold = 1047  # Default threshold
        
        # IP setup password
        self.ip_setup_password = "LBRT123!"  # Default IP setup password
    ...
    def create_password_page(self):
        ...
        password_label.grid(row=1, column=0, columnspan=2, pady=(20, 10))
        ...
        self.password_entry.grid(row=2, column=0, columnspan=2, pady=10, ipady=5)
    ...
    def validate_password(self):
        ...
        if entered_password == self.maintenance_password:
    ...
    def create_ip_setup_password_page(self):
        ...
        self.ip_setup_password_entry.grid(row=2, column=0, columnspan=2, pady=10, ipady=5)
    ...
    def validate_ip_setup_password(self):
        entered_password = self.ip_setup_password_entry.get()
        if entered_password == self.ip_setup_password:
    ...
    def set_turbo_threshold(self):
        try:
            ...
            if new_threshold > 0:
                ...
                messagebox.showinfo("Success", f"Turbo temperature threshold set to {new_threshold}")
                self.create_maintenance_page()  # Refresh the page to show new value
            else:
                messagebox.showerror("Invalid Input", "Threshold must be a positive number")
        except ValueError:
            messagebox.showerror("Invalid Input", "Please enter a valid number")
    ...
    def create_unit_monitors(self):
        ...
        for i, unit in enumerate(all_units):
            ...
            # SP Controls - only visible when maintenance mode is active
            if self.maintenance_mode_active:
            ...
            while self.monitoring_active:
                ...
                try:
                    if client.connect():
                        ...
                        # Read current value from register 401212 (only if maintenance mode is active)
                        if self.maintenance_mode_active and widgets['setpoint_value'] is not None:
